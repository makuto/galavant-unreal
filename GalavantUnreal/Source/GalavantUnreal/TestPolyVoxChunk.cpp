...
void createHeightNoise(PolyVox::SimpleVolume<uint8_t> * volData, float xOffset, float yOffset, float zOffset, float scale, int seed)
{
    ...
    for (int y = 0; y < volData->getHeight(); y++)
    {
        for (int z = 0; z < volData->getDepth(); z++)
        {
            for (int x = 0; x < volData->getWidth(); x++)
            {
                float noiseValue = testNoise.scaledOctaveNoise2d((x + xOffset) * scale, (y + yOffset) * scale, 0, volData->getHeight(), 10, 0.1f, 0.55f, 2);
...
class cell
{
	public:
		...
		cell()
		{
			...
			volData = new PolyVox::SimpleVolume<uint8_t>(PolyVox::Region(PolyVox::Vector3DInt32(0,0,0), PolyVox::Vector3DInt32(CELL_X, CELL_Y, CELL_Z)));
			std::cout << "done\n";
		}
		~cell()
		{
		...
		int generate(float newX, float newY, float newZ, float scale, int seed)
		{
			...
			const float gridSize = 100.f; // to make things easier to work with, make it so 100 unreal units (cm) = 1 noise/grid unit
			x = newX / gridSize;
			y = newY / gridSize;
			z = newZ / gridSize;
			...
			//PolyVox::CubicSurfaceExtractor< PolyVox::SimpleVolume<uint8_t> > surfaceExtractor(volData, volData->getEnclosingRegion(), &mesh);
			mesh.clear();
...
void setSurfaceMeshToRender(const PolyVox::SurfaceMesh<PolyVox::PositionMaterialNormal>& surfaceMesh, TArray<FCustomMeshTriangle>* Triangles, float scale)
{
	if (!Triangles)
		return;
	...
	for (std::vector<uint32_t>::const_iterator it = vecIndices.begin(); it != vecIndices.end(); ++it) // note that the inner loop will move forward two (three indices = 1 triangle)
	{
		...
		FCustomMeshTriangle *pCurrentTriangle = &Triangles->GetData()[iCurrentTriangle];

		if (!pCurrentTriangle || !v1 || !v2 || !v3)
			break;

		// Reverse winding order (I think unreal uses the opposite of OpenGL)
		pCurrentTriangle->Vertex2.Set(v1->position.getX() * scale, v1->position.getY() * scale, v1->position.getZ() * scale);
		pCurrentTriangle->Vertex1.Set(v2->position.getX() * scale, v2->position.getY() * scale, v2->position.getZ() * scale);
		pCurrentTriangle->Vertex0.Set(v3->position.getX() * scale, v3->position.getY() * scale, v3->position.getZ() * scale);
...
void ATestPolyVoxChunk::ConstructForPosition(FVector Position, float noiseScale, int seed, float meshScale)
{
	FCustomMeshTriangle emptyTriangle;
	...
	// Tell Unreal these are the triangles to display
...
ATestPolyVoxChunk::ATestPolyVoxChunk() : LastUpdatedPosition(0.f, 0.f, 0.f)
{
	...
	Mesh = CreateDefaultSubobject<UCustomMeshComponent>(TEXT("Mesh"));
...
void ATestPolyVoxChunk::Tick( float DeltaTime )
{
	...
	//SceneComponent.AddWorldOffset(FVector DeltaLocation, bool bSweep, FHitResult* OutSweepHitResult, ETeleportType Teleport);
